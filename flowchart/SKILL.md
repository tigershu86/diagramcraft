---
name: flowchart
description: 生成流程导向的交互式 React SVG 图。用于工作流、步骤说明、决策树、时序图、状态机、层级图和泳道图，以及包含条件、时间顺序、角色分工或状态转换的可视化请求。不要用于云架构、基础设施、微服务或软件组件拓扑；这些请求使用 arch-diagram 技能。
---
...
为 Diagramcraft 共享渲染器生成契约数据时，机器消费者必须在渲染前使用随技能包发布的 `references/diagram.schema.json` 验证数据。不使用共享渲染器时，仍可按下文生成独立 JSX。
...
边界规则：系统由哪些组件组成、部署在哪里、组件如何通信 → `arch-diagram`；事情按什么步骤发生、由谁处理、何时分支或改变状态 → `flowchart`。若请求同时包含两类信息，按用户的主要问题选择，不要同时触发两个技能。
...
const STYLE = {
  ...
  state:    { fill: "#F5F3FF", stroke: "#C4B5FD", text: "#5B21B6" },
...
function NodeShape({ node, isH, isS }) {
  ...
  return <rect x={node.x} y={node.y} width={w} height={h} rx={11}
...
- [ ] 二元决策节点有且仅有两条带明确条件的出边
- [ ] 流程图有一个开始节点和至少一个明确结束节点；状态机与层级图不套用此限制
